...
from ...monitoring.monitoring import fic_monitor_tasks, ensure_fic_task, cancel_task_safely
from .demo import cancel_demo_tasks, schedule_demo_notifications
...
async def build_notifications_panel(uid: int) -> str:
    ...
    await db.ensure_moodle_until_set(uid)
    ...
    moodle_on = bool(rec and rec.get("moodle_active"))
    m_left = db.moodle_notif_days_left(rec) if moodle_on else None
    m_tail = f" (auto-off in {m_left} day{'s' if m_left != 1 else ''})" if moodle_on and m_left is not None else ""
    return (
        f"<b>FIC:</b> {'on 🔔' if fic_on else 'off 🔕'}{tail} | "
        f"Update: {format_dt_vancouver((fic_st or {}).get('updated_at'))}\n"
        f"<b>Moodle:</b> {'on 🔔' if moodle_on else 'off 🔕'}{m_tail} | "
...
async def _render_settings(callback: CallbackQuery) -> None:
    ...
    note = (
        f"\nℹ️ FIC notifications: <b>{NOTIF_DURATION_DAYS} days</b> (auto-off). "
        f"Moodle notifications: <b>{moodle_days} days</b> (cap {MOODLE_NOTIF_MAX_DAYS}).\n"
        f"⏱️ Check interval: FIC <b>{CHECK_INTERVAL_SEC}</b> sec | Moodle <b>{MOODLE_CHECK_INTERVAL_SEC}</b> sec."
    )
...
async def cb_settings_notif(callback: CallbackQuery):
    ...
    moodle_days = min(MOODLE_NOTIF_DURATION_DAYS, MOODLE_NOTIF_MAX_DAYS)
    note = (
        f"\nℹ️ FIC: <b>{NOTIF_DURATION_DAYS} days</b>. "
        f"Moodle: <b>{moodle_days} days</b> (cap {MOODLE_NOTIF_MAX_DAYS})."
    )
...
async def cb_notif_fic_on(callback: CallbackQuery, bot: Bot):
    ...
    rec = await db.get_user(uid)
    if rec and bool(rec.get("is_demo")):
...
async def cb_notif_fic_off(callback: CallbackQuery):
    ...
    rec = await db.get_user(uid)
    if rec and not bool(rec.get("fic_active")) and not bool(rec.get("moodle_active")):
...
@router.callback_query(F.data == "notif:moodle:on")
async def cb_notif_moodle_on(callback: CallbackQuery, bot: Bot):
    await callback.answer()
    uid = callback.from_user.id
    await db.set_moodle_active(uid, True)
    rec = await db.get_user(uid)
    if rec and bool(rec.get("is_demo")):
        await schedule_demo_notifications(bot, uid)
    else:
...
@router.callback_query(F.data == "notif:moodle:off")
async def cb_notif_moodle_off(callback: CallbackQuery):
    await callback.answer()
    uid = callback.from_user.id
    await db.set_moodle_active(uid, False)
    # Stop the monitoring task only if neither source is active.
    rec = await db.get_user(uid)
    if rec and not bool(rec.get("fic_active")) and not bool(rec.get("moodle_active")):
        if bool(rec.get("is_demo")):
            await cancel_demo_tasks(uid)
